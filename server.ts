...
app.use((req, res, next) => {
  ...
  // HSTS - Force HTTPS in production (365 days)
...
// Admin Authorization Middleware for secure gateway management
export function adminAuthMiddleware(req: express.Request, res: express.Response, next: express.NextFunction) {
...
// Removed from top-level to prevent side-effects during function import
...
app.use((err: any, req: express.Request, res: express.Response, next: express.NextFunction) => {
  ...
  const message = config.nodeEnv === "production" 
    ...
    : err.message || "An unexpected error occurred inside the DCP simulation complex.";
...
async function startServer() {
  ...
  // Graceful Shutdown Logic for maintenance and deployment safety
  const shutdown = (signal: string) => {
    console.log(`[SHUTDOWN] ${signal} received. Closing HTTP server and flushing telemetry drains...`);
    server.close(() => {
      console.log("[SHUTDOWN] HTTP server closed. Cleanup complete.");
      process.exit(0);
    });
